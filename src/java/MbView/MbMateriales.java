    ...
    public String registrar() {
     ...
     public void actualizarTodo(){
        ...
        try {
            ...
            List<Materiales> listaMat = new ArrayList<>();
            listaMat =  daoMaterial.getAll(this.session);
            
           for(Materiales item:listaMat){
               item.setPreciocost(item.getPreciocost()+(item.getPreciocost()*porcentajeAumento/100));
               item.setPrecioAnonizado(item.getPrecioAnonizado()+(item.getPrecioAnonizado()*porcentajeAumento/100));
               item.setPrecioPintura(item.getPrecioPintura()+(item.getPrecioPintura()*porcentajeAumento/100));
               daoMaterial.actualizar(this.session, item);
           }
        
            System.out.println("registramos los cambios");
